...
async def rerun_task(task_id: str, goal: str, preset_id: str | None, web_search: bool) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("DELETE FROM agent_os_messages WHERE task_id=$1", uuid.UUID(task_id))
        await conn.execute("DELETE FROM agent_os_subtasks WHERE task_id=$1", uuid.UUID(task_id))
        await conn.execute(
            "UPDATE agent_os_tasks SET status='pending', result=NULL, updated_at=NOW() WHERE id=$1",
            uuid.UUID(task_id),
        )
