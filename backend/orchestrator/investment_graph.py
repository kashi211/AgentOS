...
async def analyst_node(state: InvestmentState) -> dict:
    ...
    await _save_message(state["task_id"], "analyst", "output", response)
    await _update_task_status(state["task_id"], "planning")
    return {
        "analyst_output": response,
        "status": "planning",
        "events": [{"type": "agent_output", "agent": "analyst", "content": "Bull case analysis complete"}],
    }
...
async def bear_case_node(state: InvestmentState) -> dict:
    ...
    await _save_message(state["task_id"], "bear_case", "output", response)
    await _update_task_status(state["task_id"], "executing")
    return {
        "bear_output": response,
        "status": "executing",
        "events": [{"type": "agent_output", "agent": "bear_case", "content": "Bear case analysis complete"}],
    }
...
async def synthesizer_node(state: InvestmentState) -> dict:
    ...
    await _save_message(state["task_id"], "synthesizer", "output", response)
    return {
        "synthesizer_output": response,
        "status": "reviewing",
        "events": [{"type": "agent_output", "agent": "synthesizer", "content": f"Investment memo drafted (revision {state['revision_count'] + 1})"}],
    }
...
async def risk_agent_node(state: InvestmentState) -> dict:
    ...
    await _save_message(state["task_id"], "risk_agent", "output", response)
    ...
    if passed:
        ...
        return {
            ...
            "status": "executing",
            "events": [{"type": "qa_fail", "agent": "risk_agent", "content": feedback[:200]}],
        }
