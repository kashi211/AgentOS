...
async def list_presets():
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, data FROM agent_os_custom_presets ORDER BY created_at DESC"
        )
...
async def upsert_preset(preset_id: str, body: PresetUpsert):
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO agent_os_custom_presets (id, data)
            VALUES ($1, $2::jsonb)
...
async def delete_preset(preset_id: str):
    ...
    async with pool.acquire() as conn:
        result = await conn.execute(
            "DELETE FROM agent_os_custom_presets WHERE id = $1", preset_id
        )
    if result == "DELETE 0":
