...
async def register(body: RegisterBody):
    ...
    async with pool.acquire() as conn:
        existing = await conn.fetchrow("SELECT id FROM agent_os_users WHERE email=$1", body.email.lower())
        if existing:
            raise HTTPException(400, "Email already registered")
        row = await conn.fetchrow(
            "INSERT INTO agent_os_users (email, password_hash) VALUES ($1, $2) RETURNING id, email",
            body.email.lower(),
            password_hash,
        )
    user_id = str(row["id"])
...
async def login(body: LoginBody):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, email, password_hash FROM agent_os_users WHERE email=$1",
            body.email.lower(),
        )
    if not row:
