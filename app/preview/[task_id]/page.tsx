...
const agentColor: Record<string, string> = {
  ceo: "#1d4ed8", planner: "#1e40af", developer: "#0284c7",
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#0f172a", overflow: "hidden" }}>
      ...
      <div style={{ height: 52, display: "flex", alignItems: "center", gap: 10, padding: "0 16px", borderBottom: "1px solid #1e293b", flexShrink: 0, background: "#0a0f1e" }}>
        ...
        <button
          ...
          style={{ color: theater ? "#93c5fd" : "#94a3b8", display: "flex", padding: 6, borderRadius: 6, border: `1px solid ${theater ? "#1d4ed8" : "#1e293b"}`, cursor: "pointer", background: theater ? "rgba(29,78,216,0.15)" : "transparent" }}
        >
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{ width: theater ? 0 : 220, flexShrink: 0, borderRight: theater ? "none" : "1px solid #1e293b", display: theater ? "none" : "flex", flexDirection: "column", background: "#0a0f1e", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            {files.map((f) => (
              <button key={f.path} onClick={() => selectFile(f.path)} style={{ width: "100%", textAlign: "left", padding: "9px 14px", display: "flex", alignItems: "center", gap: 8, cursor: "pointer", border: "none", borderLeft: selectedFile === f.path ? "2px solid #1d4ed8" : "2px solid transparent", background: selectedFile === f.path ? "rgba(29,78,216,0.12)" : "transparent" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          {files.length === 0 ? (
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <Loader2 size={20} style={{ color: "#1d4ed8", animation: "spin 0.8s linear infinite" }} />
            </div>
          ) : mode === "demo" && hasDemo ? (
            <iframe
              ref={iframeRef}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, padding: "6px 12px", borderBottom: "1px solid #1e293b", flexShrink: 0, background: "#0a0f1e" }}>
                ...
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 6, border: "none", cursor: isDirty && !saving ? "pointer" : "default", background: isDirty && !saving ? "#1d4ed8" : "#1e293b", color: isDirty && !saving ? "#fff" : "#475569", fontSize: 12, fontWeight: 600 }}
                >
            ...
            {threads.map((thread) => (
              <div key={thread.id}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
                  <div style={{ background: "#1d4ed8", color: "#fff", borderRadius: "12px 12px 4px 12px", padding: "8px 12px", fontSize: 13, maxWidth: "85%", lineHeight: 1.5 }}>
                    {thread.message}
                  </div>
                </div>
                {/* Agent events */}
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  {!thread.done && !thread.failed && thread.events.length > 0 && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px" }}>
                      <Loader2 size={12} style={{ color: "#1d4ed8", animation: "spin 0.8s linear infinite" }} />
          ...
          <div style={{ padding: "12px 14px", borderTop: "1px solid #1e293b", flexShrink: 0 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
              ...
              <button
                ...
                style={{ width: 36, height: 36, borderRadius: 10, background: editing || !editInput.trim() ? "#1e293b" : "#1d4ed8", border: "none", cursor: editing || !editInput.trim() ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
              >
